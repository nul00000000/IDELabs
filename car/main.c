...
int main() {
	...
	for(i = 0;; i++) {
		...
		if(carpet) {
			...
			setWheel(0, (-0.55f + (diffDrive > 0 ? diffDrive * DIFF_COEF : 0)) * speed); //the polarity is now correct
			setWheel(1, (-0.55f + (diffDrive < 0 ? -diffDrive * DIFF_COEF : 0)) * speed);
			//setWheel(0, 0);
			//setWheel(1, 0);
