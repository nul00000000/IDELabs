...
int TIMER_A0_PWM_Init(uint16_t freq, double percentDutyCycle, uint16_t pin)
{
	...
	DEFAULT_PERIOD_A0[pin] = SystemCoreClock/16/freq;
	TIMER_A0->CCR[0] = SystemCoreClock/16/freq;
...
// Outputs: none
int TIMER_A2_PWM_Init(uint16_t freq, double percentDutyCycle, uint16_t pin)
{

	//stop PWM
	uint16_t dutyCycle;
	TIMER_A2->CTL &= ~(BIT4 | BIT5);
	// Timer A0.1
	if (pin >= 1 || pin <= 4)
	{
		P5->SEL0 |= (BIT5 << pin);
	...
	// save the period for this timer instance
	// DEFAULT_PERIOD_A0[pin] where pin is the pin number
	DEFAULT_PERIOD_A2[pin] = SystemCoreClock/16/freq;
	TIMER_A2->CCR[0] = SystemCoreClock/16/freq;
	//TIMER_A0->CCR[0] = (uint16_t) (period * percentDutyCycle);
	
	// TIMER_A0->CCTL[pin]
	//start PWM
  TIMER_A2->CCTL[pin] |= (BIT7 | BIT6 | BIT5);
  ...
  TIMER_A2->CCR[pin] = dutyCycle;
	...
	TIMER_A2->EX0 &= ~(BIT0 | BIT1 | BIT2);
	TIMER_A2->EX0 |= BIT0;
	
	// Timer CONTROL register
	// TIMER_A0->CTL
	//set clock source
	TIMER_A2->CTL &= ~(BIT8 | BIT9); 
	TIMER_A2->CTL |= BIT9;
	//set clock divider
	TIMER_A2->CTL |= (BIT6 | BIT7);
	//clear timer
	TIMER_A2->CTL |= BIT2;
	//start up counting
	TIMER_A2->CTL |= BIT4;
