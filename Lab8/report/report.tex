...
\begin{abstract}
Knowing the heart rate of a human being is something which is
...
\section{Background}
    ...
    the OPB745 sensor was used. The method used for measuring change in blood 
    volume involves shining an infrared LED into the finger, and measuring the
    amount of that light reflected out of the finger. This is represented as
    the voltage across the phototransistor within the OPB745. In this way, the
    amount of blood flowing through the finger can be measured. If the sensor
    records a dark signal which then becomes light, it can be considered a
    pulse of the heart as the blood rushed past. By measuring the time between
    these pulses, the patient's heart rate can be calculated. Ideally, this is
    done with an average of multiple measurements. Basing a calculation off of
    only one pulse could be problematic as the voltage changes by very small
    amounts. This requires significant levels of amplification. If the voltage
    change is so small that the sensor doesn't pick it up, the calculation
    could be completely incorrect. Averages limit this possibility.
...
latency.
    ...
    The fully constructed circuit is shown in simulation in Fig. \ref{fig:opb_sim}
    and on the breadboard in Fig. \ref{fig:opb_real}.
    \begin{figure}[h]
        \centering
